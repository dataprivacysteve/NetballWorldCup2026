...
| 4. Identity documents            | Complete | Passport or national-ID upload, full file decoding/signature validation, restricted LOC/federation viewing, document-bound LOC operational decisions, replacement-race protection, retained evidence for official federation approval, and attributed view history. |
| 5. LOC and federation review     | Complete | Exactly one full LOC officer account for operational validation/accreditation; a separate World Netball read-only login for official eligibility inspection; no federation mutation routes; transactional credential issuance and attributed audit history. |
...
| LOC operations         | One `loc_officer` can see the complete registration registry and make operational registration/identity/roster and accreditation decisions on behalf of the event. |
| Federation review      | A dedicated `federation_viewer` can inspect submitted teams, player/person IDs, profiles and retained identity evidence for World Netball eligibility approval. The projection is read-only, excludes contact and consent-party data, and audits team-record and identity-document views. |
...
- A self-cleaning live workflow verified person creation, photo upload,
  passport upload, restricted and audited reviewer access, manual LOC
  verification, and retained evidence for the official federation review.
