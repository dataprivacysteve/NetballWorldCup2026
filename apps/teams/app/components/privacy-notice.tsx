        ...
        <div className="fixed inset-0 z-[120] flex items-center justify-center bg-navy-deep/75 p-3 backdrop-blur-sm sm:p-6" onMouseDown={(event) => { if (event.target === event.currentTarget) setOpen(false); }}>
          <section role="dialog" aria-modal="true" aria-labelledby="privacy-title" className="flex max-h-[92vh] w-full max-w-4xl flex-col overflow-hidden rounded-2xl border border-white/20 bg-white shadow-[0_30px_100px_rgba(5,9,30,0.45)]">
            ...
            <div className="enterprise-table-scroll overflow-y-auto px-5 py-5 text-sm leading-6 text-ink-soft sm:px-7 sm:py-6">
              <p>This notice explains how personal information is collected, used, accessed, retained and deleted when teams, officials and authorised event personnel use the GameDay registration, accreditation and operations platform. The Organising Committee facilitates registration and match operations for the Netball World Cup Qualifier — Americas, World Netball is the official team-eligibility authority, and SportsBB provides authorised platform administration and technical support.</p>
              ...
              <NoticeSection title="4. Identity documents and restricted handling">
                <p>Passport and national-ID files are used only for eligibility and identity verification by the authorised LOC reviewer and a named, read-only World Netball reviewer. They are not displayed on the public website, included on credentials, or made available to ordinary team users or GameDay officials. The restricted file is retained through World Netball’s approval and any appeal window, then securely deleted under the approved retention schedule. The platform also retains the minimum verification outcome, reviewer, date, reason or correction note, and audit record needed to demonstrate the decision.</p>
              </NoticeSection>
              ...
              <NoticeSection title="6. Who may access information">
                <p>Access is limited by role. Each delegation can access only its own records. The authorised LOC officer can perform operational registration, roster and accreditation checks. A named World Netball reviewer can view submitted teams, player profiles and restricted identity evidence for official eligibility approval, but cannot edit records or perform LOC actions; identity-document views are logged. SportsBB administrators can configure and support the platform but do not use restricted documents for unrelated purposes. Assigned GameDay and gate personnel receive only the information required for their operational role. Information may also be disclosed where required by law, safeguarding obligations or an authorised event-governance process.</p>
              </NoticeSection>
